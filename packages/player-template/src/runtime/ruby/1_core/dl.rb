  ...
  class << self
    ...
    def __read_memory(address, length)
      ...
      if bitmap_data
        ...
        return bitmap_data[:bitmap].__rgba_pixel_data.byteslice(offset, length) || ''.b
      end
    ...
    def __write_memory(address, length, value)
      ...
      replacement = value.byteslice(0, length) || ''.b
      replacement = replacement.ljust(length, "\x00")
      data[offset, length] = replacement
    ...
    def __bitmap_layout(bitmap)
      @bitmap_layouts[bitmap.object_id] ||= begin
        ...
        @bitmap_data_addresses[data] = { address: data, bitmap: bitmap }
    ...
    def __bitmap_data_for_address(address)
      ...
      @bitmap_data_addresses.each_value do |entry|
        bitmap = entry[:bitmap]
        data_length = bitmap.width * bitmap.height * 4
        return entry if normalized_address >= entry[:address] && normalized_address < entry[:address] + data_length
      end
