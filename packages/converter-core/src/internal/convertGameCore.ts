...
export const convertGameCore = async (options: ConvertGameOptions): Promise<ConvertGameCoreResult> => {
  ...
  const walkedSourceFiles = await walkGameFiles(srcDir);
  ...
  const manifest = buildManifestJson(materializedResources, {
...
const writeGameOutput = async (
  ...
  await rm(join(context.outDir, SAFE_ASSET_DIRNAME), { recursive: true, force: true });
  await rm(join(context.outDir, PACK_ASSET_DIRNAME), { recursive: true, force: true });
  await copySourceFiles(context, copySourceFilePaths);
  await copySafeAssetFiles(context);
  await writePackFiles(context.packBuilder, context.outDir);
  await writeFile(join(context.outDir, MANIFEST_FILENAME), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
};

const copySourceFiles = async (context: ConversionContext, sourceFiles: string[]) => {
  for (const relativePath of sourceFiles) {
    const source = join(context.srcDir, relativePath);
    const target = join(context.outDir, relativePath);
...
const copySafeAssetFiles = async (context: ConversionContext) => {
  for (const convertedFile of context.files) {
    ...
    const source = join(context.srcDir, convertedFile.sourcePath);
    const target = join(context.outDir, convertedFile.outputPath);
